...
*   **SSO 登录**：使用当前北航 SSO 身份；校园网自动直连，校外自动通过 VPN。
*   **用户信息展示**：获取并展示用户的基本信息。
*   **公告通知**：接收并查看重要通知公告。
...
1.  在设置中点击“登录 iClass SSO”，应用自动选择直连或 VPN 登录入口。
2.  默认显示当天的课程，也可以点击日历图标选择特定日期。
3.  在课程列表中找到需要签到的课程。
4.  点击对应课程卡片上的“签到”按钮即可完成签到。
5.  点击左上角菜单按钮打开侧边栏，可以查看公告、进行设置或验证。

课表与签到不再要求输入学号。每次操作重新检测 iClass 直连地址的可达性，单次操作内固定访问路径；SSO 失效时先重新登录，不切换到备用接口。

远程命令：`getSchedule` 只要求 `date`；`signCourse` 只要求 `courseScheduleId`，均使用当前 SSO 身份。
